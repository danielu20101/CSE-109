...
int createUrlFromLink (URL_t * url, char * link) {
  ...
  // If we didn't find "://", then we return 203 error
  if (index >= size) return -1;
  if (index >= size || link[index++] != ':') return 203;
  if (index >= size || link[index++] != '/') return 203;
  if (index >= size || link[index++] != '/') return 203;

  // Check if the protocol is valid
  // If we didn't find http/https, then we return 202 error
...
int searchHrefs (char *userid, char *password, char *server, char *pathname, int * count) {
  ...
  while (findNextLink(fd, linkBuffer, 2048) != 1) {
    ...
    URL_t url;
    int result = createUrlFromLink(&url, linkBuffer);
    if (result > 0) {
      fprintf(stderr, "Specification error\n");
      return result;
    } else if (result < 0) {
      Fclose(fd);
      // Delete the file
